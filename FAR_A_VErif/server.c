...
void attaquer(int x, int y){
  ...
  resetPlacement(x,y);
  mapJeu.positions.mercenaire[idMercenaire-1] = (position){0,0};
  setMercenaireMort(idMercenaire,monitor.mercenaires[idMercenaire-1]); 
  //setMercenaireMort(idMercenaire,monitor.mercenaires[]);
...
void modificationPosition(char * tokenOrientation,char * id, Mercenaire m) {
    int idT = atoi(id);
    int oldX;
    int oldY;
    int mur;
  ...
  while (recv(csock, action, 64, 0) > 0) {
    ...
    if (strcmp(tokenId, "8") == 0) {
      ...
      if (strcmp(tokenAction, "deplacer") == 0 && (mercenaire.id == monId || monId == 7)) {
        ...
        modificationPosition(tokenOrientation,id,mercenaire); //
      ...
      //  verifPositionVillageois(mercenaire);
        ...
        attaquer(monitor.oedipe.x,monitor.oedipe.y);
      }
    }
